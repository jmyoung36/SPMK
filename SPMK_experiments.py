...
K_lin = np.genfromtxt(metadata_dir + 'kernels/sub_1pt5/K_lin.csv', delimiter=',')
...
#
#lin_results = np.zeros((10,))
#SPMK_results = np.zeros((10,))
...
#            accs = repeated_k_cv(K, labels, clf, 20, 20, 'accuracy')
...
#    K_test = K[test_index, :][:, train_index]josien pluim
#    train_labels = np.array(IXI_metadata['SEX_ID'].tolist())[train_index]
#    test_labels = np.array(IXI_metadata['SEX_ID'].tolist())[test_index]
#    clf.fit(K_train, train_labels)
#    preds = clf.predict(K_test)
#    SPMK_results[ind] = metrics.accuracy_score(test_labels, preds)
#    print 'SPMK accuracy = ' + str(metrics.accuracy_score(test_labels, preds))
#    
...
#    clf.fit(K_train, train_labels)
#    preds = clf.predict(K_test)
#    lin_results[ind] = metrics.accuracy_score(test_labels, preds)
#    
#    
#    
#    ind += 1
...
#print SPMK_results
#print np.mean(SPMK_results)
#print lin_results
#print np.mean(lin_results)
...
for train_index, test_index in kf:
    ...
    for i in range(1, 6) :
        ...
        for j in range(1, 8) :
            ...
            # first try intersection kernel            
            if (n_intensity_levels == 16 and n_histogram_levels == 7) or n_intensity_levels == 32 :
                K = np.genfromtxt(metadata_dir + 'kernels/sub_1pt5/SPMK_intersection_' + str(n_intensity_levels) + '_' + str(n_histogram_levels) + '_3.csv', delimiter=',')
            else :
                K = np.genfromtxt(metadata_dir + 'kernels/sub_1pt5/SPMK_intersection_' + str(n_intensity_levels) + '_' + str(n_histogram_levels) + '.csv', delimiter=',')
            K = K[train_index, :][:, train_index]
            labels = np.array(IXI_metadata['AGE'].tolist())[train_index]
            ...
            # next try bhattacharyya kernel
            if (n_intensity_levels == 16 and n_histogram_levels == 7) or n_intensity_levels == 32 :
                K = np.genfromtxt(metadata_dir + 'kernels/sub_1pt5/SPMK_bhattacharyya_' + str(n_intensity_levels) + '_' + str(n_histogram_levels) + '_3.csv', delimiter=',')
            else :
                K = np.genfromtxt(metadata_dir + 'kernels/sub_1pt5/SPMK_bhattacharyya_' + str(n_intensity_levels) + '_' + str(n_histogram_levels) + '.csv', delimiter=',')
            K = K[train_index, :][:, train_index]
            #maes = repeated_k_cv(K, labels, reg, 20, 20, 'mean_absolute_error')
    ...
    print 'best number of intensity levels is ' + str(best_n_intensity_levels)    
    print 'best number of histogram levels is ' + str(best_n_histogram_levels)     
    K = np.genfromtxt(metadata_dir + 'kernels/sub_1pt5/SPMK_' + selected_kernel_type[ind] + '_' + str(int(best_n_intensity_levels)) + '_' + str(int(best_n_histogram_levels)) + '.csv', delimiter=',')
    K_train = K[train_index, :][:, train_index]
